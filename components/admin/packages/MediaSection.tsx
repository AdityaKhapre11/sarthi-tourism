          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {gallery.map((img, index) => (
              <div key={index} className="relative aspect-square rounded-xl border border-white/10 overflow-hidden group bg-white/5">
                {img ? (
                  <>
                    <Image src={img} alt={`Gallery ${index + 1}`} fill unoptimized className="object-cover" onError={(e: React.SyntheticEvent<HTMLImageElement>) => (e.currentTarget.style.display = 'none')} />
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity gap-2">
                      <Button type="button" variant="ghost" size="icon" onClick={(e: React.MouseEvent) => { e.stopPropagation(); onGalleryImageClick(index); }} className="text-white hover:bg-white/20">
                        <Upload className="w-4 h-4" />
                      </Button>
                      <Button type="button" variant="ghost" size="icon" onClick={(e: React.MouseEvent) => { e.stopPropagation(); onRemoveGalleryImage(index); }} className="text-red-400 hover:bg-red-500/30">
                        <X className="w-4 h-4" />
                      </Button>
                    </div>
                  </>
